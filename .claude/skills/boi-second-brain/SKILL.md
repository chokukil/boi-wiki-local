...
If a source folder is configured and `agent_session_check` is `true`, inspect it at session start with the agent's native file tools. In `explicit-only` mode, inspect it only after the user explicitly asks to organize or review the folder. Compute SHA256 for supported email, Markdown/text, CSV, PDF, and image files; ignore temporary, incomplete, unsupported, and excluded files. The stored processing state is advisory: the current file path, size, modified time, and SHA256 are authoritative.
