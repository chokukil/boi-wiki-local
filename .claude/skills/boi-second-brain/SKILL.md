...
For administrator/reference verification only, `scripts/local_wiki.py curate-knowledge` is an explicit Local-only CLI: it previews by default and requires `--apply-local` before writing. It validates the declared Local source and evidence SHA256 values, Local-path provenance, and case identity; it never starts a runtime service or submits Remote changes. `--current-path` is optional because a Profile may have no approved Current baseline. Curation identity spans active `notes/knowledge` and `notes/review`: a state transition archives the stale active artifact under Local Private history, removes its active index link, and leaves exactly one active knowledge or Review item without changing Current.
...
An explicit type request such as `웹 클립만 처리해줘` or `새 PDF만 처리해줘` limits only the current run. Leave unselected new hashes pending; do not mark them complete. The same SHA256 observed under different paths or apparent types produces one canonical Local knowledge item, with every observation preserved in provenance. Every readable new unique hash uses `templates/source-knowledge-template.md` and becomes reusable `OKF 0.1 + BoI Profile 0.1-local` knowledge with claims, decisions or constraints, evidence, counterevidence, unknowns, and the Local/Remote boundary. In `auto-curate`, make conflict-free source knowledge available to Local search immediately; create a Review Queue item only when the material-change boundary above is met. A Review candidate never increases or overwrites the approved Current revision before human approval. If there are no new unique hashes, return `no-change` without creating or editing a report, document, index, log, progress state, or revision.
...
- Investigation question: separate compiled Wiki pages, supporting evidence, and optional remote BoI references. Read the most relevant compiled pages before opening raw evidence. Treat direct answer, evidence, counterevidence, unknowns, next checks, confidence, and citations as internal analytical requirements, not a mandatory visible outline. Compose the result in the natural structure the question needs. For every counterclaim, state whether it changes the current reviewed conclusion; an unverified counterclaim must not silently override reviewed knowledge. If those analytical requirements cannot be supported, say the Wiki is insufficient and propose an intake or review step; do not fill the gap from model memory. Save only a reviewed answer with at least one Local citation.
...
For an ordinary investigation or Local synthesis, follow `references/answer-surface-contract.md` and `references/citation-surface-contract.md`. When the user explicitly asks for approved Current knowledge, additionally follow `references/local-current-query.md`; use `references/current-knowledge-manifest.example.json` and `references/query-pack-v4.example.json` only as implementation or audit shapes. The Query Pack is an in-memory agent contract, not an application or resident runtime.

Route an ordinary question from reviewed Local knowledge first. When no reviewed answer is available, the ordinary question automatically continues with bounded Local discovery: reread only directly relevant Local Markdown, verify its SHA256, and label it in plain user language. When directly relevant Local Markdown is found, answer from it before optional Shared or public discovery; do not search the web merely because no reviewed answer exists. A request such as `검토한 내용만으로 답해줘` keeps the strict reviewed-only path and never falls back to review or raw material. Every question is read-only: discovery does not approve, promote, or write Knowledge, Manifest, Review, History, progress state, or Remote data.

The default presentation style is `natural-expert`: conclusion-first, professional and conversational Korean, with depth adapted to the question rather than a fixed seven-part template. After grounding is complete, run at most one meaning-preserving presentation repair. The critic may improve ordering, clarity, and rhythm, but it may not repeat retrieval, change the evidence set, add facts, write knowledge, or alter Current, Review, or promotion state.

For administrator or benchmark validation, produce the sibling local generation receipt after the agent composes the Markdown answer. Bind the exact question, deterministic query plan, selected citation display map, each public original's evidence ID + exact SHA256 + expected exact URL or accepted stable scholarly identifier, answer bytes, `natural-expert` composer, critic pass count, and every material paragraph's declared claim-to-citation relationship. Carry that same original-identity binding through Query Pack, citation display, receipt, and benchmark evaluation; URL shape alone is never authority. A receipt does not certify semantic truth. Missing, stale, or mismatched receipts fail the Answer Surface axis independently of retrieval and original-evidence checks.
