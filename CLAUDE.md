...
- If MCP is available, prefer `dictionary_resolve` for terminology before interpreting acronyms, `ontology_search` for broad SOP/Event/Action/Dictionary/runtime lookup, `workflow_definitions_search` and `workflow_definition_deduplicate` as internal duplicate checks before proposing new shared connections, `boi_agent_chat` for page-aware questions, `agent_inbox` for assigned tasks, and `boi_search` only for document-only BoI search. Present user-facing links as BoI Wiki, SOP, Event, or Action destinations, not as WorkflowDefinition pages.
