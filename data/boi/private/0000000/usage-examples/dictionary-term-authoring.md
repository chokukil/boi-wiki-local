...
5. 필요하면 `term_kind`와 parent relation을 정리한다. 세부 test/mode/variant 용어는 상위 개념 없이 Team/Public canonical로 공유하지 않는다.
6. `data/boi/private/{7자리사번}/dictionary/response-trend.md`에 Local Private dictionary BoI를 저장한다.
7. `index.md`와 `log.md`를 업데이트하고 self-check 결과를 보고한다.
...
대량 후보는 개별 용어마다 code를 바꾸지 않고 `keep`, `replace_with_canonical`, `split_into_terms`, `alias_to_existing`, `exclude`, `needs_parent_curation` 같은 판단을 manifest나 promotion draft에 남긴다.
