...
모든 새 후보는 review queue에 둡니다. `알아서 정리`도 후보 생성까지만 자동화하며 사용자 승인 전에는 현재 지식 revision을 증가시키지 않습니다. 신규 unique hash가 없으면 `no-change`로 끝내고 보고서·문서·index·log·revision을 만들거나 바꾸지 않습니다.
