...
Second Brain의 품질은 파일 개수나 Graph 모양이 아니라, 질문했을 때 **주제 지식에서 출발해 실제 원문 근거와 반증까지 추적할 수 있는가**로 판단합니다. BoI Wiki Local의 질의 계약은 관련 파일을 한꺼번에 던지는 대신 다음 순서를 만듭니다.
...
7. `citations`: 사용자 답변에는 `[1]`부터 `[5]`까지 실제 원문 근거만 연결합니다. 기본 답변에는 전체 SHA256과 절대 경로를 노출하지 않습니다. 상세 근거·검토·감사 요청에서만 상대 경로, exact SHA256과 계보를 보여줍니다.
...
- 인용이 내부 근거 매핑에 없거나 원문의 SHA256이 다름
- AI가 합성한 주제 노트나 검색 결과를 원문 출처처럼 인용함
- 같은 원문 근거가 서로 다른 표시 번호를 받음
